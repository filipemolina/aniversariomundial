						...
						foreach ($cols as $column_name => $column_value) {
							switch ($column_name) {
								...
								case 'image'					:
									...
									if ($column_name != "image" || ($column_name == "image" && !empty($show_avatars))) {
								...
								switch ($column_name) {
									...
									case 'registered'						:
										...
										<?php if (apply_filters($this -> pre . '_admin_subscribers_registeredcolumn', true)) : ?>
											<td><label for="checklist<?php echo $subscriber -> id; ?>"><?php echo (empty($subscriber -> registered) || $subscriber -> registered == "N") ? '<span class="newsletters_error">' . __('No', $this -> plugin_name) : '<span class="newsletters_success"><a href="' . get_edit_user_link($subscriber -> user_id) . '">' . __('Yes', $this -> plugin_name) . '</a>'; ?></span></label></td>
										<?php endif; ?>
										<?php
										break;
									case 'mandatory'						:
